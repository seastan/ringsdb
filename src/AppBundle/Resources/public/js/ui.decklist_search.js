(function(ui, $) {
    ...
    ui.add_custom_pack_section = function add_custom_pack_section() {
        ...
        // Update counter to include newly added custom pack checkboxes.
        ui.handle_checkbox_change();

        // Keep counter in sync when custom packs are toggled.
    ...
    ui.setup_typeahead = function() {
        ...
        $('#card_to_exclude').on('typeahead:selected typeahead:autocompleted', function(event, data) {
            ...
            var line = $('<p class="fg-' + card.sphere_code + '" style="padding: 3px 5px; border-radius: 3px; border: 1px solid silver"><button type="button" class="close" aria-hidden="true">&times;</button><input type="hidden" name="cards_to_exclude[]" value="' + card.code + '">' + app.data.display_name(card) + ' <small><i>' + card.type_name + '</i></small></p>');
            line.on({
                click: function(event) {
                    line.remove();
                }
