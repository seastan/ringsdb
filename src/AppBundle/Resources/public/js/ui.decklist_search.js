(function(ui, $) {
    ...
    ui.setup_typeahead = function() {
        ...
        $('#card_to_exclude').on('typeahead:selected typeahead:autocompleted', function(event, data) {
            ...
            var line = $('<p class="fg-' + card.sphere_code + '" style="padding: 3px 5px; border-radius: 3px; border: 1px solid silver"><button type="button" class="close" aria-hidden="true">&times;</button><input type="hidden" name="cards_to_exclude[]" value="' + card.code + '">' + card.name + ' <small><i>' + card.type_name + '</i></small></p>');
            line.on({
                click: function(event) {
                    line.remove();
                }
