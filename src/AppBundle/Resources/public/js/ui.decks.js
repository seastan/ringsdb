(function ui_decks(ui, $) {
    ...
    ui.setup_typeahead = function() {
        ...
        $('#card_used').on('typeahead:selected typeahead:autocompleted', function(event, data) {
            ...
            var line = $('<p class="fg-' + card.sphere_code + '" style="padding: 3px 5px; border-radius: 3px; border: 1px solid silver"><button type="button" class="close" aria-hidden="true">&times;</button><input type="hidden" name="cards_used[]" value="' + card.code + '">' + card.name + ' <small><i>' + card.type_name + '</i></small></p>');
            line.on({
        ...
        $('#card_not_used').on('typeahead:selected typeahead:autocompleted', function(event, data) {
            ...
            var line = $('<p class="fg-' + card.sphere_code + '" style="padding: 3px 5px; border-radius: 3px; border: 1px solid silver"><button type="button" class="close" aria-hidden="true">&times;</button><input type="hidden" name="cards_not_used[]" value="' + card.code + '">' + card.name + ' <small><i>' + card.type_name + '</i></small></p>');
            line.on({
                click: function(event) {
                    // Remove the line in the html 
                    line.remove();
